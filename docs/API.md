...
```json
{
  ...
  "notes": "- 修正登入逾時\n- 優化掛機穩定度",
  "files": [
    { "kind": "zip", "name": "TMD_Runner.zip", "url": "https://tmd-run.netlify.app/download/afk/zip", "size": 62914560, "sha256": "e3b0c4..." }
  ],
  "backup_url": null
}
```
...
| `files[].kind` | 目前只有 `zip`，對應 `products.json` 的 `files` |
...
Release 有附上的檔案才會出現在 `files`。如果發版時附的 zip 檔名和 `products.json` 不一樣，`files` 會是空的。
...
例如 `/download/afk/zip`。回 `302` 轉到 GitHub 產生的暫時下載網址（幾分鐘內有效）。程式端用 `HttpClient` 下載時預設會自動跟隨轉址。
...
3. 下載 `kind == "zip"` 的檔案到暫存資料夾，算 SHA256 比對 `sha256`。
4. 解壓縮。zip 根目錄直接是 exe 和 `config.json`，不能多包一層資料夾。
5. 和目前執行檔同名的檔案存成 `.new`（Windows 不能覆蓋執行中的 exe），其他檔案直接覆蓋到程式資料夾（設定檔只含預設值）。
6. 啟動一個小批次檔，等主程式結束後把 `.new` 改名成正式檔名並重新啟動。
7. 範例程式碼在 `docs/updater-sample/UpdateChecker.cs`。
